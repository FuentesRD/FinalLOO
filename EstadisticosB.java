...
public class EstadisticosB {

    private ArrayList <Double> datos;
    private int numDatos;
    
    public EstadisticosB(ArrayList<Double> arr, int dim){
        datos = new ArrayList<>();
        ...
        if(arr.size() == 1){
            ...
            double numeroInter;
            for(int i=0; i<dim; i++){
                numeroInter = numero.nextDouble(0,1);
                numeroInter *= 100;
                datos.add(numeroInter);
            } 
        } else {
            for(Double num : arr){
                datos.add(num);
    ...
    private void ordenar(ArrayList<Double> lista1){
